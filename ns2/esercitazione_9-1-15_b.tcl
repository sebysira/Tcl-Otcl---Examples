...
proc finish {} {
	...
	exec xgraph graph.tr -t "Variation queue" -x "queueLimit" -y "timeTest" &;						#Execute XGRAPH
    exit 0
}
...
proc simulation {} {
	global fd fg times timeTotal timeTest timeSlice nTest totTest maxAck_D_A maxAck_A_D agent_A_send agent_D_send ftp_A_D ftp_D_A bytes_A_D bytes_D_A B C queueLimit
	set ns [Simulator info instances]
	...
	if {$ackRic_A_D >= $maxAck_A_D && $ackRic_D_A >= $maxAck_D_A} {
		...
		puts $fg "$queueLimit($nTest) $timeTest"
		...
		$ns queue-limit $B $C $queueLimit($nTest);
		$ns queue-limit $C $B $queueLimit($nTest);
...
array set queueLimit {1 3 2 6 3 9 4 12 5 15 6 18 7 21 8 24 9 27 10 30 11 33 12 36 13 39 14 42 15 45 16 48 17 51 18 54 19 57 20 60}

set lossrate 0.005
loss $lossrate $B $C

# punto a dell' esercizio
$ns queue-limit $B $C $queueLimit(1);
$ns queue-limit $C $B $queueLimit(1);
